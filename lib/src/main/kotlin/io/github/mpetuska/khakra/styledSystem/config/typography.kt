@file:JsModule("@chakra-ui/styled-system/dist/esm/config/typography")

package io.github.mpetuska.khakra.styledSystem.config

import io.github.mpetuska.khakra.styledSystem.core.*
import react.*


/**
 * Types for typography related CSS properties
 */
public external interface TypographyProps : RProps {
  /**
   * The CSS `font-weight` property
   */
  public var fontWeight: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `line-height` property
   */
  public var lineHeight: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `letter-spacing` property
   */
  public var letterSpacing: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `font-size` property
   */
  public var fontSize: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `font-family` property
   */
  public var fontFamily: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `text-align` property
   */
  public var textAlign: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `font-style` property
   */
  public var fontStyle: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `word-break` property
   */
  public var wordBreak: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `overflow-wrap` property
   */
  public var overflowWrap: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `text-overflow` property
   */
  public var textOverflow: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `text-transform` property
   */
  public var textTransform: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `white-space` property
   */
  public var whiteSpace: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `text-decoration` property
   */
  public var textDecoration: dynamic
    get() = definedExternally
    set(value) = definedExternally
  
  /**
   * The CSS `text-decoration` property
   */
  public var textDecor: dynamic
    get() = definedExternally
    set(value) = definedExternally
}

public external val typography: Parser
public external val typographyParser: Parser
